...
Minecraft to Discord uses only `api.messages().observeAcceptedMessages(...)`. The bridge checks `origin.kind() == MINECRAFT`, resolves a stable `ChannelId` mapping, suppresses duplicate `messageId` values in a bounded TTL receipt cache, sanitizes mentions, and submits a bounded Discord REST operation.
...
The common Discord layer owns JDA lifecycle, ready-time queue capacity, outbound capacity, mention suppression, surrogate-safe 2,000-character fitting, bounded publish retry, bounded observer receipts, and shutdown drain. A process crash between a successful Discord send and receipt state is not claimed to be exactly-once; the receipt is intentionally bounded and in-memory.
