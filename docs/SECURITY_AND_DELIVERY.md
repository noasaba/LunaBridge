...
Observer receipts are bounded in memory with a TTL. They suppress duplicate `AcceptedMessage.messageId` notifications within the process and evict the oldest receipt at capacity rather than blocking new messages, but the bridge does not claim durable exactly-once delivery across a process crash.
