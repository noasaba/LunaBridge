...
public final class HandshakeMessages {
    ...
    public record Hello(String serverId, long issuedAt, byte[] nonce, byte[] mac) {
        public Hello { nonce = nonce.clone(); mac = mac.clone(); }
    }
    ...
    public record ClientState(Hello hello, byte[] serverKey) { public ClientState { serverKey = serverKey.clone(); } }
    public record IssuedChallenge(Challenge challenge, byte[] serverKey) { public IssuedChallenge { serverKey = serverKey.clone(); } }

    public static ClientState begin(String serverId, byte[] serverKey, Clock clock) {
        byte[] nonce = random(NONCE_BYTES);
        long now = clock.millis();
        byte[] mac = mac(serverKey, "hello", serverId, now, nonce);
        return new ClientState(new Hello(serverId, now, nonce, mac), serverKey);
    }
    ...
    public static IssuedChallenge challenge(Hello hello, byte[] expectedServerKey, Clock clock) throws ProtocolException {
        ...
        if (!MessageDigest.isEqual(hello.mac(), mac(expectedServerKey, "hello", hello.serverId(), hello.issuedAt(), hello.nonce()))) {
    ...
    public static byte[] encode(Hello value) { return encode(out -> { writeString(out, value.serverId()); out.writeLong(value.issuedAt()); writeBytes(out, value.nonce()); writeBytes(out, value.mac()); }); }
    ...
    public static Hello decodeHello(byte[] bytes) throws ProtocolException { return decode(bytes, in -> new Hello(readString(in, 64), in.readLong(), readBytes(in, NONCE_BYTES), readBytes(in, MAC_BYTES))); }
    ...
    private static byte[] mac(byte[] key, Object... parts) {
        ...
        try (DataOutputStream out = new DataOutputStream(bytes)) {
            ...
            for (Object part : parts) {
                ...
                else if (part instanceof byte[] array) writeBytes(out, array);
    ...
    private static <T> T decode(byte[] bytes, Reader<T> reader) throws ProtocolException { if (bytes.length > 4096) throw new ProtocolException(ProtocolException.Code.LIMIT_EXCEEDED, "handshake too large"); try (DataInputStream in = new DataInputStream(new ByteArrayInputStream(bytes))) { T result = reader.read(in); if (in.available() != 0) throw new IOException("trailing bytes"); return result; } catch (IOException error) { throw new ProtocolException(ProtocolException.Code.MALFORMED, error.getMessage()); } }
    ...
    private static String readString(DataInputStream in, int limit) throws IOException { int length = in.readUnsignedByte(); if (length > limit) throw new IOException("string too large"); return new String(in.readNBytes(length), StandardCharsets.UTF_8); }
    ...
    private static byte[] readBytes(DataInputStream in, int expected) throws IOException { int length = in.readUnsignedByte(); if (length != expected) throw new IOException("invalid byte length"); return in.readNBytes(length); }
