...
public final class HandshakeMessages {
    ...
    private static <T> T decode(byte[] bytes, Reader<T> reader) throws ProtocolException { if (bytes == null) throw new ProtocolException(ProtocolException.Code.MALFORMED, "missing handshake"); if (bytes.length > 4096) throw new ProtocolException(ProtocolException.Code.LIMIT_EXCEEDED, "handshake too large"); try (DataInputStream in = new DataInputStream(new ByteArrayInputStream(bytes))) { T result = reader.read(in); if (in.available() != 0) throw new IOException("trailing bytes"); return result; } catch (IOException error) { throw new ProtocolException(ProtocolException.Code.MALFORMED, error.getMessage()); } }
